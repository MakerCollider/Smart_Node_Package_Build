...
// 32: extern int cameraInit ( cameraConfig config , cameraCb cb ) ;
...
// 34: extern int cameraOnData ( int toggle ) ;
...
// 55: extern int faceDetectInit ( faceDetectConfig config , faceDetectNumCb ncb , faceDetectImgCb icb , faceDetectAllCb acb ) ;
Handle<Value> faceDetectInitV8(const Arguments &args) {
    ...
    cbArray[2] = Persistent<Function>::New(Local<Function>::Cast(args[2]));
    faceDetectImgCb arg2;;
    arg2 = cbFunc2;
    
    
    V8_ASSERT(args[3]->IsFunction(), "args[3] parameters error!");
    cbArray[3] = Persistent<Function>::New(Local<Function>::Cast(args[3]));
...
// 57: extern int faceDetectOnData ( unsigned long imgHandle ) ;
Handle<Value> faceDetectOnDataV8(const Arguments &args) {
    HandleScope scope;
    
    // Convert V8 parameters to C++
    
    V8_ASSERT(args[0]->IsNumber(), "args[0] parameters error!");
    unsigned long arg0 = (unsigned long)args[0]->NumberValue();
    
        
    // Call C++ function
    int ret = (int)faceDetectOnData(arg0);
...
// 58: extern int faceDetectRelease ( ) ;
...
// 78: extern int img2Base64Init ( img2Base64Config config , img2Base64Cb cb ) ;
...
// 79: extern int img2Base64OnData ( unsigned long imgHandle ) ;
Handle<Value> img2Base64OnDataV8(const Arguments &args) {
    ...
    // Call C++ function
    int ret = (int)img2Base64OnData(arg0);
...
// 80: extern int img2Base64Release ( ) ;
Handle<Value> img2Base64ReleaseV8(const Arguments &args) {
    HandleScope scope;
        
    // Call C++ function
    int ret = (int)img2Base64Release();
